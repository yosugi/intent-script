...
* レベル1（形式的）: 完全に形式的な構文で、厳格なYAML構造として実装
  ```yaml
  User:
...
* レベル3（自然言語）: 自然言語主体の表現、AIによる意図解析に依存
  ```text
...
特に、形式的に厳密な記述（レベル1）を使用する場合でも、コメントを通じて自然言語での意図説明を追加できる：

```yaml
# レベル1の形式的記述だが、コメントで意図を自然言語で補足している
User:
